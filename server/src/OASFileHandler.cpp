...
bool FileHandler::initialize(const std::string &cacheDirectoryPath) {
  ...
  if (0 !=
      stat(cacheDirectoryPath.c_str(), &fileInfo)) // stat returns 0 on success
  {
    oas::Logger::error("FileHandler - Cache directory setup failed");
    oas::Logger::errorf("Intended cache directory was \"%s\"",
                        cacheDirectoryPath.c_str());
    return false;
  } else if (!S_ISDIR(fileInfo.st_mode)) // macro in sys/stat.h to determine if
                                         // directory
  {
    oas::Logger::errorf("FileHandler - Cache directory path \"%s\" does not "
...
bool FileHandler::writeFile(const std::string &filename, const char *data,
                            unsigned int size) {
  ...
  if (fileOut.bad()) {
    oas::Logger::errorf(
        "FileHandler - Writing to file \"%s\" failed. Data may be corrupted.",
        fileOutPath.c_str());
    fileOut.close();
    return false;
  }
...
void *FileHandler::readFile(const std::string &filename, int &fileSize) {
  ...
  if (0 != stat(filePath.c_str(), &fileInfo)) // stat returns 0 on success
  {
    oas::Logger::warnf("FileHandler - Could not access \"%s\"",
                       filePath.c_str());
    return NULL;
  }
...
#ifdef XML_CONFIG
bool FileHandler::loadXML(const std::string &filename,
  ...
  if (!fp) {
    oas::Logger::errorf("FileHandler - Unable to open XML file \"%s\".",
                        filename.c_str());
    return false;
  }
  ...
  mxml_node_t *tree = mxmlLoadFile(NULL, NULL, fp);

  if (!tree) {
    oas::Logger::errorf(
        "FileHandler - Could not process \"%s\" as an XML file!",
        filename.c_str());
    fclose(fp);
    return false;
  }
  ...
  this->_root = mxmlFindElement(this->_tree, this->_tree, root.c_str(), NULL,
                                NULL, MXML_DESCEND_FIRST);

  if (!this->_root) {
    oas::Logger::errorf(
        "FileHandler - Could not find \"%s\" as root of the XML file!",
        root.c_str());
    fclose(fp);
    return false;
  }
...
bool FileHandler::findXML(const char *name, const char *attr, const char *value,
                          std::string &output) {
  if (!this->_root) {
  ...
  mxml_node_t *node = mxmlFindElement(this->_root, this->_root, name, attr,
                                      value, MXML_DESCEND_FIRST);

  if (node) {
    if (mxml_node_t *child = mxmlGetFirstChild(node)) {
      auto result = mxmlGetOpaque(child);
      if (result) {
        output = result;
      } else {
        output.clear();

...
oas::Server::Server() : _audioHandler(oas::AudioHandler::getInstance()) {
  _serverInfo = NULL;
}
...
bool oas::Server::_readConfigFile(int argc, char **argv) {
  ...
  if (fh.findXML("gui", NULL, NULL, gui) && gui.size()) {
    if (!gui.compare("off") || !gui.compare("false") || !gui.compare("no") ||
        !gui.compare("disable") || !gui.compare("disabled")) {
      oas::Logger::logf(
  ...
  std::string cacheDirectory =
      *config->value<std::string>("OAS", "cache_directory", cachedefault);
  long port = *config->value<int64_t>("OAS", "port", 31231);
  this->_serverInfo = new ServerInfo(cacheDirectory, port);

  std::string audioDevice =
      *config->value<std::string>("OAS", "audio_device", "");
  this->_serverInfo->setAudioDeviceString(audioDevice);
...
void *oas::Server::_run(void *parameter) {
  ...
  while (1) {
    ...
    if (SocketHandler::isConnectedToClient())
      timeOut +=
          Time(0.0005); // 0.5 ms -> maximum of ~2000 loop iterations per second

...
bool oas::Server::_readConfigFile(int argc, char **argv)
{
    ...
    // Check if the command line arg is unreasonably large to stop simple buffer
    // overflow attacks
    if (configFilePath.size() > 1024)
    {
        oas::Logger::logf(
            "The specified config file location is unreasonably large.\n");
        return false;
    }
    ...
    {
        oas::Logger::errorf(
            "The specified configuration file at '%s' does not exist.\n",
    ...
    // The config file MUST contain a cache directory and the port number the
    // server will listen on Find these two items in the XML file
    if (fh.findXML("cache_directory", NULL, NULL, cacheDirectory) && fh.findXML("port", NULL, NULL, port))
    {
    ...
    else
    {
        if (!cacheDirectory.size())
            this->_fatalError("Could not retrieve cache directory information from "
                              "configuration file.");
        else if (!port.size())
            this->_fatalError("Could not retrieve port number information from "
                              "configuration file.");
    }
    ...
    {
        if (!gui.compare("off") || !gui.compare("false") || !gui.compare("no") || !gui.compare("disable") || !gui.compare("disabled"))
        {
            oas::Logger::logf(
                "The GUI has been disabled, as requested by the configuration file.");
    ...
    config::Access access(host, cluster);
    auto config = access.file("config");
    std::string cachedefault = "cache";
    if (auto home = getenv("HOME"))
    {
        cachedefault = home;
        cachedefault += "/.cache/oas";
    }
    std::string cacheDirectory = *config->value<std::string>("OAS", "cache_directory", cachedefault);
    long port = *config->value<int64_t>("OAS", "port", 31231);
    this->_serverInfo = new ServerInfo(cacheDirectory, port);

    std::string audioDevice = *config->value<std::string>("OAS", "audio_device", "");
    this->_serverInfo->setAudioDeviceString(audioDevice);

    bool gui = *config->value<bool>("OAS", "gui", true);
    ;
    this->_serverInfo->setGUI(gui);
#endif
...
void oas::Server::_processMessage(const Message &message)
{
    ...
    case oas::Message::MT_GHDL_FN:
        ...
        // 1) If the file is already loaded into a buffer, the buffer is reused in
        // the new source 2) Else look at filesystem to see if file exists
        //      2a) If exists, load audio file into a new buffer, then create new
        //      source 2b) Else file does not exist, send  "-1" response
        //            oas::Logger::logf("GHDL %s", message.getFilename());
        newSource = _audioHandler.createSource(message.getFilename());
        if (-1 == newSource)
            oas::Logger::logf(
                "Server was unable to generate new audio source for file \"%s\".",
                message.getFilename().c_str());
        else
            oas::Logger::logf("New sound source created for \"%s\". (Sound ID = %d)",
                message.getFilename().c_str(), newSource);
        oas::SocketHandler::addOutgoingResponse(newSource);
        break;
    case oas::Message::MT_WAVE_1I_3F:
        newSource = _audioHandler.createSource(
            message.getIntegerParam(), message.getFloatParam(0),
            message.getFloatParam(1), message.getFloatParam(2));
        if (-1 == newSource)
            oas::Logger::logf(
                "Server was unable to generate a new audio source based on the "
                "waveform:\n"
            ...
            oas::Logger::logf(
                "New sound source created based on the waveform:\n"
                "    waveshape = %d, freq = %.2f, phase = %.2f, duration = %.2f",
                message.getIntegerParam(), message.getFloatParam(0),
                message.getFloatParam(1), message.getFloatParam(2));
        oas::SocketHandler::addOutgoingResponse(newSource);
    ...
    case oas::Message::MT_SSEC_HL_1F:
        _audioHandler.setSourcePlaybackPosition(message.getHandle(),
            message.getFloatParam(0));
        break;
    case oas::Message::MT_SSPO_HL_3F:
        _audioHandler.setSourcePosition(
            message.getHandle(), message.getFloatParam(0), message.getFloatParam(1),
            message.getFloatParam(2));
        break;
    case oas::Message::MT_SSVO_HL_1F:
        _audioHandler.setSourceGain(message.getHandle(), message.getFloatParam(0));
        break;
    case oas::Message::MT_SSLP_HL_1I:
        _audioHandler.setSourceLoop(message.getHandle(), message.getIntegerParam());
        break;
    case oas::Message::MT_SSVE_HL_1F:
        oas::Logger::warnf("SSVE using speed instead of velocity is deprecated!");
        _audioHandler.setSourceSpeed(message.getHandle(), message.getFloatParam(0));
        break;
    case oas::Message::MT_SSVE_HL_3F:
        _audioHandler.setSourceVelocity(
            message.getHandle(), message.getFloatParam(0), message.getFloatParam(1),
            message.getFloatParam(2));
        break;
    ...
    case oas::Message::MT_SSDI_HL_3F:
        _audioHandler.setSourceDirection(
            message.getHandle(), message.getFloatParam(0), message.getFloatParam(1),
            message.getFloatParam(2));
        break;
    case oas::Message::MT_SSDV_HL_1F_1F:
        _audioHandler.setSourceDirection(message.getHandle(),
            message.getFloatParam(0));
        _audioHandler.setSourceGain(message.getHandle(), message.getFloatParam(1));
        break;
    case oas::Message::MT_SPIT_HL_1F:
        _audioHandler.setSourcePitch(message.getHandle(), message.getFloatParam(0));
        break;
    case oas::Message::MT_FADE_HL_1F_1F:
        _audioHandler.setSourceFade(message.getHandle(), message.getFloatParam(0),
    ...
    case oas::Message::MT_SLOR_3F_3F:
        _audioHandler.setListenerOrientation(
            message.getFloatParam(0), message.getFloatParam(1),
            message.getFloatParam(2), message.getFloatParam(3),
            message.getFloatParam(4), message.getFloatParam(5));
        break;
...
void oas::Server::initialize(int argc, char **argv)
{
...
#ifdef FLTK_FOUND
    if (getServerInfo()->useGUI() && !oas::ServerWindow::initialize(argc, argv, &oas::Server::terminate))
    {
    ...
    {
        std::stringstream s;
        s << "Could not initialize the File Handler for cache directory "
          << this->_serverInfo->getCacheDirectory() << "!";
    ...
    {
        std::stringstream s;
        s << "Could not initialize the Audio Handler for device "
          << this->_serverInfo->getAudioDeviceString() << "!";
        std::string str = s.str();
        _fatalError(str.c_str());
    }
    ...
    {
        std::stringstream s;
        s << "Could not initialize the Socket Handler for port "
          << this->_serverInfo->getPort() << "!";
        std::string str = s.str();
        _fatalError(str.c_str());
    }
    ...
    if (getServerInfo()->useGUI())
        threadError = pthread_create(&this->_serverThread, &threadAttr,
            &oas::Server::runServer, NULL);
    // Without the GUI
    else
        threadError = pthread_create(&this->_serverThread, &threadAttr,
            &oas::Server::runServerNoGUI, NULL);
#else
    // Without the GUI
    threadError = pthread_create(&this->_serverThread, &threadAttr,
        &oas::Server::runServerNoGUI, NULL);
...
void *oas::Server::_run(void *parameter)
{
    ...
    while (1)
    {
        ...
        // If a client is connected, use a very short timeout allowing for fast
        // updates
        ...
        // If there are no incoming messages, populateQueueWithIncomingMessages()
        ...
        while (!messages.empty())
        {
            ...
            //            oas::Logger::logf("Server processed message \"%s\"",
            //            nextMessage->getOriginalString().c_str());
            delete nextMessage;
            messages.pop();
...
#ifdef FLTK_FOUND
    // Fl::run() puts all of the FLTK window rendering on this current thread
    // (main thread) It will only return when program execution is meant to
    // terminate

...
/**
 * Although these max sizes are somewhat arbitrary, they should still prevent
 * overflow.
 */
...
public:
   ...
   * @return Pointer to allocated data. The caller needs to free it when
   * finished
   ...
   * @param output If an element was found, then this will be the resulting
   * string.
   * @retval True Element was found
   * @retval False Element not found
   */
  bool findXML(const char *name, const char *attr, const char *value,
               std::string &output);
#endif

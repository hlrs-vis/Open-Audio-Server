...
public:
     ...
     *  CLOCK_MONOTONIC: Fast access clock that cannot be modified by programs,
     * monotonic since some unspecified starting point. Best used for timing
     * intervals. CLOCK_REALTIME: System-wide realtime clock that can be set with
     * appropriate privileges.
     */
    typedef clockid_t ClockType;
    static constexpr const ClockType OAS_CLOCK_MONOTONIC = CLOCK_MONOTONIC;
    static constexpr const ClockType OAS_CLOCK_REALTIME = CLOCK_REALTIME;

    /**
     * @brief Update with the value of the current time as recorded by the
     * specified clock
     */
    ...
    /**
     * @brief Reset the value in the clock so that it no longer records any valid
     * time.
     *
     */
    inline void reset() { _time.tv_sec = _time.tv_nsec = 0; }
     ...
     */
    inline const struct timespec getTime() const { return _time; }

    /**
     * @brief Return the seconds portion of the stored value
     */
    inline const long int getSeconds() const { return _time.tv_sec; }

    /**
     * @brief Return the nanoseconds portion of the stored value
     */
    inline const long int getNanoseconds() const { return _time.tv_nsec; }
    ...
    /**
     * @brief Constructor that initializes based on a floating point

...
Time Time::operator+(const Time &other) const { return Time(*this) += other; }

Time Time::operator-(const Time &other) const { return Time(*this) -= other; }

bool Time::operator>(const Time &other) const
{
    if (getSeconds() > other.getSeconds() || (getSeconds() == other.getSeconds() && getNanoseconds() > other.getNanoseconds()))
        return true;
    else
        return false;
}
...
bool Time::operator==(const Time &other) const
{
    if (getNanoseconds() == other.getNanoseconds() && getSeconds() == other.getSeconds())
    {
        return true;
    }
    else
